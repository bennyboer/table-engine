...
export class ImageCellRenderer implements ICanvasCellRenderer {
	 ...
	 */
	private readonly _imageCache: Map<string, CanvasImageSource> = new Map<
		string,
		CanvasImageSource
	>();
	...
	public render(
		...
		if (cell.value !== null) {
			...
			const image: CanvasImageSource | null = this._getImageIfLoaded(
				value.src
			);
			if (!!image) {
				...
				if (width > bounds.width || height > bounds.height) {
					...
					const overlapX: number =
						width > bounds.width ? width - bounds.width : 0;
					const overlapY: number =
						height > bounds.height ? height - bounds.height : 0;
	...
	public getCopyValue(cell: ICell): string {
		...
		if (!!image) {
			const bounds: IRectangle = this._engine
				.getCellModel()
